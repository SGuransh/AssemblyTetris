...
Check_for_row_completion:
    # Check for all the rows if they are complete
    # If they are complete send the above pixels 10 down each
    # Making a for loop for iterating over the values from 0 to 11 (12 points in each row)
    lw $t0, displayaddress
    addi $t0, $t0, 256500   # Get the address of the last register at (125, 250)
    lw $t1, displayaddress
    addi $t1, $t1, 52244    # intialize address register to 5, 51
    lw $t7, 0($t1)      # This holds the current address that we are examining
    li $t2, 0       # column number we are at
    Loop_1_start_for_row_completion:
        beq $t2, 200, Loop_1_end_for_row_completion
        li $t3, 0       # this is the loop iteration variable, row number we are at
        li $t4, 1       # this is the boolean which tells if all the values are white or not
        Loop_2_start_for_row_completion:
            lw $t7, 0($t1)
            sll $t5, $t3, 2     # shift for row
            sll $t6, $t2, 10    # shift for column
            add $t7, $t7, $t5
            add $t7, $t7, $t6
            lw $t8, 0($t7)      # this is the value stored here
            bne $t8, 0xffffff, not_white
                li $t4, 0
                j Loop_1_start_for_row_completion
            not_white:
            beq $t2, 120, Loop_2_end_for_row_completion
            addi $t3, $t3, 10
            j Loop_2_start_for_row_completion        
        Loop_2_end_for_row_completion:
        
        # If at the end of the loop the value of the variable is 1 then we know that we have to bring down every pixel above
        bne $t4, 1, Continue_row_completion
            lw $a0, 0($t7)
            lw $a1, 0($t3)
            lw $a2, 0($t2)
            jal bring_things_down
        addi $t2, $t2, 10
        Continue_row_completion:
        
        j Loop_1_start_for_row_completion
    Loop_1_end_for_row_completion:
jr $ra

# this takes the address of the end point of the previous column of completion
bring_things_down:
    # we have the last point of checking 
